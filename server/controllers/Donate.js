...
const getAllDonations = (req, res) => {
    ...
    db.query('Select * from recorddonation group by req_id', (err, results) => {
        if(err){
            return res.json({error:err})
        }else{
...
const getDonationById = (req, res) =>{
    ...
    db.query('SELECT * from recorddonation where req_id = ?', [req_id], (err, results)=>{
        if(err){
            return res.json({error:err})
        }else{
            return res.send(results);
...
module.exports ={ Donate, postDonate,getAllDonations, getDonationById}
