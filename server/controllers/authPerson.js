...
const RegisterP = (req,res)=>{
    ...
    const {cnic, password, fname, lname, blood_group, weight, frequencyDonation, IsDiabetes, IsHypetitus, contactno,  postalCode, Area, Block, City, file} = req.body
    
    // if empty value passed
    if(!cnic){
        // error handling 
        console.log('please enter cnic error');
    }
    //1
            ...
            db.query('select loc_id from personlocation where postalCode = ? AND Area = ? AND Block = ? AND City = ?', [postalCode, Area,Block, City], (err, results)=>{
                ...
                db.query('select cnic from person where cnic = ?', [cnic], async (err, results)=>{
                    if(err){
                        ...
                        console.log("person me aya");
                        hashpassword = await bcrypt.hash(password, 8);
                        //   5
                       db.query('insert into person (cnic, password, fname , lname , blood_group, weight, frequencyDonation, IsDiabetes, IsHypetitus, loc_id, file) values (?,?,?,?,?,?,?,?,?,?,?) ', [cnic, hashpassword, fname, lname, blood_group, weight, frequencyDonation, IsDiabetes, IsHypetitus, loc_id, file], (err, result) => {
                       
                        if (err) {
                            console.log("error in person" + err);
                           return res.json({error:err})
                        } else {
...
const LoginP = (req, res)=>{
    ...
    if(!cnic || !password){
        return res.status(404).send("please enter cnic or password ")
    }
    else{
